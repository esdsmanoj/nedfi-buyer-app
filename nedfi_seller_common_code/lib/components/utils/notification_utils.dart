...
import 'package:nedfi_seller_common_code/components/utils/NotificationTopics.dart' as notification;
import 'package:nedfi_seller_common_code/pages/marketPlace/nedfi_products/incentive_screen.dart';
import 'package:uuid/uuid.dart';

import '../../app_imports.dart';
import '../../model/home_page_model.dart';
import '../../pages/helpdesk/helpdesk_screen.dart';
import '../../pages/marketPlace/demand/BuyerDemandMainScreen.dart';
import '../../pages/marketPlace/nedfi_products/trade_product_details.dart';
...
class NotificationUtils {
  ...
  Future createNotification(final message, final params, {bool isStatus = false}) async {
    AwesomeNotifications().createNotification(
      ...
      notificationLayout: isStatus ? NotificationLayout.BigPicture : NotificationLayout.BigText,
    ));
  }
  ...
  /// Listening when app is in the foreground state.
  ...
  Future<void> createWaterReminderNotification(int hashCode, String map) async {
    await AwesomeNotifications().createNotification(
      ...
      actionButtons: [NotificationActionButton(key: 'accept', label: 'Accept'), NotificationActionButton(key: 'reject', label: 'Reject')],
    );
  ...
  Future handleAllNotification(final navigatorKey, BuildContext context) async {
    ...
    try {
      AwesomeNotifications().setListeners(
        ...
        final route = (payload["route"] ?? "").toString().toLowerCase();

        print("payload: $payload");

        if (route == notification.NotificationTopics.CHAT.toString().toLowerCase()) {
          Navigator.push(
            navigatorKey!.currentContext ?? context,
            MaterialPageRoute(
              builder: (context) => ChatScreen(
                farmerName: payload["sold_to"]?.toString() ?? '',
                buyerId: payload["farmer_id"]?.toString().toLowerCase() ?? '',
                from: "bid",
                prodId: payload["id"]?.toString() ?? '',
                tradeProductBiddingId: payload["bidding_id"]?.toString() ?? '',
              ),
            ),
          );
          ...
          Navigator.push(
            context,
            MaterialPageRoute(builder: (ctx) => TradeProductDetails(productId: payload["id"]?.toString() ?? '', productTitle: "")),
          );
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (ctx) => IncentiveScreen(
                buyerName: payload["buyer_name"]?.toString() ?? '',
                bidderId: payload["bidder_id"]?.toString() ?? '',
                bidderDate: payload["bidder_date"]?.toString() ?? '',
              ),
            ),
          );
        } else if (route == "demand") {
          Navigator.push(context, MaterialPageRoute(builder: (ctx) => const BuyerDemandMainScreen()));
        } else if (route == notification.NotificationTopics.Notice.toString().toLowerCase()) {
        ...
        } else if (route == notification.NotificationTopics.Announcement.toString().toLowerCase()) {
          var notice = AnnouncementData(
            id: payload["id"]?.toString() ?? '',
            title: '',
            description: '',
        ...
        } else if (route == notification.NotificationTopics.order.toString().toLowerCase()) {
        ...
        } else if (route == notification.NotificationTopics.Blog.toLowerCase()) {
          final blogData = Blogs(
            blogsId: payload["id"]?.toString() ?? '',
            logo: payload["image"]?.toString() ?? '',
            blogsTagsId: '',
            blogsTypesId: '',
            blogsTitle: payload["title"]?.toString() ?? '',
            blogsSubTitle: '',
            id: '',
            blogsSubDescription: payload["body"]?.toString() ?? '',
            blogsDescription: payload["body"]?.toString() ?? '',
            blogsCreatedOn: '',
            blogsTypesName: '',
            blogsTypesLogo: '',
            blogsTypesMobIcon: '',
        ...
        } else if (route == notification.NotificationTopics.product.toLowerCase()) {
          Navigator.push(context, MaterialPageRoute(builder: (context) => AddProductDetailsScreen()));
        } else if (route == notification.NotificationTopics.myOrders.toLowerCase()) {
  ...
  Future<void> listenerEvent(BuildContext context, Function callBack, String? advisoryID, Function(bool) stateCallBack) async {
    try {
      ...
      //           MaterialPageRoute(
      //               builder: (context) =>
      //                   VideoMeetingIFrame(farmerId: video.farmer_id, userId: userId ?? "", meetingID: video.meeting_link ?? "", name: HeaderSingleton().paramsMaps!.firstName ?? "")));
