...
import 'package:nedfi_seller_common_code/model/home_page_model.dart';
import 'package:nedfi_seller_common_code/model/trade_product_model/statistics_filter.dart';
import 'package:nedfi_seller_common_code/model/trade_product_model/statistics_report_model.dart';
import 'package:nedfi_seller_common_code/model/user_model/User.dart' as data;
import 'package:nedfi_seller_common_code/pages/about_us/terms_condition.dart';
import 'package:nedfi_seller_common_code/pages/helpdesk/helpdesk_screen.dart';
import 'package:nedfi_seller_common_code/pages/marketPlace/incentive/MyIncentiveScreen.dart';
import 'package:nedfi_seller_common_code/pages/marketPlace/marketable_surplus/MarketableSurplusScreen.dart';
import 'package:nedfi_seller_common_code/pages/marketPlace/nedfi_products/nedfi_advertisement/nedfi_advertisement.dart';
import 'package:nedfi_seller_common_code/pages/marketPlace/product_screen/MarketMainScreen.dart';
import 'package:nedfi_seller_common_code/pages/partner/PartnerListingScreen.dart';
import 'package:nedfi_seller_common_code/pages/webView/PWAIframe.dart';
...
class HelperUtils {
  ...
  String currentLat = "", currentLog = "", city = "";
  ...
  /// Getting splash images from server.
  ...
  Future initLocation(BuildContext context,Function(Tuple2<String, String>) refreshState, Function state, {bool? isWeatherActive = false}) async {
    try {
      refreshState.call(Tuple2(HeaderSingleton().lat, HeaderSingleton().lng));
      LocationData _locationData;

      // Directly check and request location service permission
      _serviceEnabled = await location.serviceEnabled();
      if (!_serviceEnabled) {
        _serviceEnabled = await location.requestService();
        if (!_serviceEnabled) {
          return;  // Location service is not enabled, can't proceed
        }
      }
      ...
      if (_permissionGranted == PermissionStatus.granted) {
        ...
        // _showMessage(context, "Permission granted. You can now use location-based features.");
      } else if (_permissionGranted == PermissionStatus.denied) {
        print("PermissionStatus.denied");
        isLocation.value=true;
        // Permission is denied, show explanation before requesting permission
        //_showExplanationAndRequest(context);
      } else if (_permissionGranted == PermissionStatus.deniedForever) {
        print("PermissionStatus.deniedForever");
        isLocation.value=true;
        // Permission is permanently denied, show a dialog to guide the user to settings
      ...
      if (_locationData != null) {
        ...
        // Fallback to stored location if the device is unable to get location
    ...
    // <string>we need access to your loaction to help you find the nearest commodity quicly and efficiently</string>
  ...
  // Function to show an explanation dialog before requesting permission
  Future<void> _showExplanationAndRequest(BuildContext context) async {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text("This app requires access to your location to provide certain features. Please grant the permission."),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.of(context).pop(); // Close the explanation dialog
                handlePermissionRequest(context); // Request permission
              },
...
// Function to handle the permission request
  Future handlePermissionRequest(BuildContext context) async {
    Location location = Location();

    // Request permission directly using the Location package
    ...
    if (permissionStatus == PermissionStatus.granted) {
      ...
      // Permission not granted, show an in-app message guiding the user to enable permissions
      _showPermissionGuide(context);
    }
  }

// Function to show a dialog guiding the user to settings if permission is permanently denied
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text("Location permission was permanently denied. Please enable it from the app settings."),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
                per.openAppSettings(); // Opens the app settings screen
              },
...
// Function to show an in-app message guiding the user to enable permissions manually
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text("Location permission is required for location-based commodities. Please enable it in the settings."),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
  ...
  // Open the settings page where the user can enable location permission
  Future<void> _openSettings() async {
    final Uri settingsUrl = Uri.parse('app-settings:'); // Open the app settings page directly
    if (await canLaunch(settingsUrl.toString())) {
      await launch(settingsUrl.toString());
    } else {
  ...
  Future getWeather(String lat, String log, Function refreshState) async {
    try {
      print("objectssss");
      print("https://nominatim.openstreetmap.org/reverse?format=json&lat=$lat&lon=$log");
  ...
  Future getProductReport(Function call, {String? year = "", String month = "", String day = ""}) async {
    try {
      final response = await APIService.postAPIMethod(url: ApiURL.getProductReport, params: {"user_id": userId, "year": year, "month": month, "day": day});
  ...
  Future getTrendingProduct(Function call, {String? year = ""}) async {
    try {
      ...
      if (response.statusCode == 200) {
        treandingproduct.value = res.data ?? [];
        call.call();
      }
    } catch (e) {
      isLoading.value = false;
  ...
  /// Performing seller action for buyer trade product.
  Future performSellerAction(final params, Function call, BuildContext context, {required bool selfSold}) async {
    try {
      final response = await APIService.postAPIMethod(url: selfSold ? ApiURL.selfSold : ApiURL.sellerAction, params: params);
  ...
  /// Getting list of cities present on server.
  ...
  /// Choose files for selecting from devices.
  ...
  /// Getting color code statuses for app.
  ...
  /// Navigating between screens as per the on tap operations.
  Future navigateToScreens({List<SeasonData>? seasonData, required BuildContext context, required String mapKey, dynamic params, WeatherModel? weather, String? url, Function? call}) async {
    ...
    if (mapKey.toLowerCase() == 'logout') {
      ...
      return launchURL(url: HeaderSingleton().privacyPolicy ?? "");
  ...
  /*///Create dynamic linking to the farmer account.
...
/// Sending OTP to the particular users that are registered on the app.
...
/// Getting the new formatting dates for the labels.
...
/// Getting the new formatting MONTH for the labels.
...
/// Getting the product information for the trade listing..
Future getProductInformation(BuildContext context, String productId, Function callback) async {
  try {
    final response = await APIService.postAPIMethod(url: ApiURL.getTradeProducts, params: {'user_id': userId, 'start': '1', 'prod_cat_id': '', 'id': productId});
    ...
    if (result.success == 1) {
      ...
      printLongString(result.data.toString());
    } else {
      productData.value = result.data;
...
void printLongString(dynamic text, {int chunkSize = 1000}) {
  final pattern = RegExp('.{1,$chunkSize}'); // Match chunks of max chunkSize
...
/// Add rating for the buyer listing who bids for the product.
Future addRating(String productId, String buyerId, String ratingId, Function callback, BuildContext context) async {
  try {
    final response =
        await APIService.postAPIMethod(url: ApiURL.productRating, params: {'buyer_id': buyerId, /* 'trade_product_id': productId,*/ 'rating_id': ratingId, 'seller_id': userId.toString()});
    final result = jsonDecode(response.body);
...
/// Deleting trade product details as per the user.
...
/// Checking the tickets list as the user present on the HELPDESK SERVER.
...
/// HELPS to logout from the server.
