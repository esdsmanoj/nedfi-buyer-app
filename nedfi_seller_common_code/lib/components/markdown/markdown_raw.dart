...
import 'html_document.dart' as md;
...
class _MarkdownBodyRawState extends State<MarkdownBodyRaw> {
  ...
  void _buildMarkdownCache() {
    ...
    SyntaxHighlighter syntaxHighlighter = widget.syntaxHighlighter ??  _DefaultSyntaxHighlighter(markdownStyle!.code!);

    _linkHandler?.dispose();
    _linkHandler =  _LinkHandler(widget.onTapLink!);
...
class _Renderer implements md.NodeVisitor {
  ...
  void visitText(md.Text text) {
    _MarkdownNodeList topList = _currentBlock!.stack!.last as _MarkdownNodeList;
  ...
  bool visitElementBefore(md.Element element) {
    ...
    if (_isBlockTag(element.tag!)) {
      ...
      List<_MarkdownNode> styleElement = <_MarkdownNode>[ _MarkdownNodeTextStyle(style, linkInfo!)];
      if (_currentBlock==null) {
  ...
  void visitElementAfter(md.Element element) {
    ...
    if (_isBlockTag(element.tag!)) {
      if (_currentBlock!.stack!.length > 0) {
        _MarkdownNodeList? stackList = _currentBlock!.stack!.first as _MarkdownNodeList?;
      ...
      if (_currentBlock!.stack!.length > 1) {
        _MarkdownNodeList? poppedList = _currentBlock!.stack!.last as _MarkdownNodeList?;
...
class _MarkdownNodeTextStyle extends _MarkdownNode {
  _MarkdownNodeTextStyle(this.style, [this.linkInfo ]);
  TextStyle style;
  _LinkInfo? linkInfo;
}
...
class _Block {
  ...
  Widget build(BuildContext context) {
    ...
    if (subBlocks!.length > 0) {
      ...
      TextSpan? span = _stackToTextSpan( _MarkdownNodeList(stack!));
      contents =  RichText(text: span!);
    ...
    if (tag == 'blockquote') {
      ...
      padding =  EdgeInsets.all(markdownStyle!.blockquotePadding!);
    } else if (tag == 'pre') {
      decoration = markdownStyle!.codeblockDecoration;
      padding =  EdgeInsets.all(markdownStyle!.codeblockPadding!);
    }
  ...
  TextSpan? _stackToTextSpan(_MarkdownNode stack) {
    ...
    if (stack is _MarkdownNodeList) {
      ...
      _MarkdownNodeTextStyle? styleNode = list[0] as _MarkdownNodeTextStyle?;
      ...
      return  TextSpan(style: style, children: children, recognizer: recognizer, text: text);
    }
