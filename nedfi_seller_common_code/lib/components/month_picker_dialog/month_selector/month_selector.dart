...
class MonthSelector extends StatefulWidget {
  const MonthSelector({super.key, required this.onMonthSelected, required this.controller});
...
class MonthSelectorState extends State<MonthSelector> {
  ...
  Widget build(BuildContext context) {
    return PageView.builder(
      ...
      itemBuilder: (final BuildContext context, final int page) => MonthYearGridBuilder(
  ...
  void _onPageChange(final int page) {
    _blocked = !(page - 1 > 0 && page + 1 < widget.controller.monthPageCount - 1);
  ...
  void initialize() {
    widget.controller.monthPageController = PageController(
      initialPage: widget.controller.localFirstDate == null ? widget.controller.selectedDate.year : widget.controller.selectedDate.year - widget.controller.localFirstDate!.year,
    );
    Future<void>.delayed(
      Duration.zero,
      () {
        Provider.of<MonthUpDownPageProvider>(context, listen: false).changePage(
          ...
          widget.controller.localFirstDate == null
              ...
              : widget.controller.localFirstDate!.year + widget.controller.monthPageController!.page!.toInt(),
          widget.controller.monthPageController!.page!.toInt() < widget.controller.monthPageCount - 1,
