import 'package:nedfi_seller_common_code/components/dialog_component/animated_snack_bar.dart';
...
abstract class MultipleSnackBarStrategy {
  ...
  List<AnimatedSnackBar> onAdd(List<AnimatedSnackBar> snackBars, AnimatedSnackBar self);
}
...
class ColumnSnackBarStrategy implements MultipleSnackBarStrategy {
  ...
  double computeDy(List<AnimatedSnackBar> snackBars, AnimatedSnackBar self) {
    ...
    if (index != -1) {
      bool isDesktop = MediaQuery.of(snackBars[index].info.key.currentContext!).size.width > 600;

      final olderBars = snackBars.where(
        (element) {
          bool isMobileAndSamePosition = (element.mobileSnackBarPosition == self.mobileSnackBarPosition && !isDesktop);
          bool isDesktopAndSamePosition = (element.desktopSnackBarPosition == self.desktopSnackBarPosition && isDesktop);

          return (isMobileAndSamePosition || isDesktopAndSamePosition) && (element.info.key.currentState?.isVisible ?? false) && element.info.createdAt.isBefore(self.info.createdAt);
        },
      ).toList();

      return olderBars.fold<double>(0, (initialValue, element) {
        final box = element.info.key.currentContext!.findRenderObject() as RenderBox;
  ...
  List<AnimatedSnackBar> onAdd(List<AnimatedSnackBar> snackBars, AnimatedSnackBar self) {
    ...
    if (index != -1) {
      ...
      bool shouldRemove(AnimatedSnackBar element) {
        ...
        return element.info.createdAt.isBefore(self.info.createdAt) && (isMobileAndSamePosition || isDesktopAndSamePosition);
      }
