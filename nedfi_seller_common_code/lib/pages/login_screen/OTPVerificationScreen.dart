...
import 'package:nedfi_seller_common_code/pages/onboarding_screen/WhiteLableSplashScreen.dart';
...
class _OTPVerificationScreenState extends State<OTPVerificationScreen> {
  ...
  /// Setting the timer for resending the OTP.
  void setCountDown() {
  ...
  /// Helps to verifying the entered OTP
      ...
      try {
        ...
        if (data.success == 1) {
          ...
          if (HeaderSingleton().isUserRegister == "1" && isProfileCompleted == "1") {
            await SharePrefsHelper.getInstance(context)?.saveBoolValue("isLogin", true);
            switch (HeaderSingleton().routeName) {
              case 'famrut':
                checkDomain();
                break;
              case 'nedfi':
        ...
        print("getLoginOTP e "+e.toString());
        isLoading.value = false;
        setState(() {});
      }
    }
  ...
  dynamic checkDomain() {
    return Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (BuildContext context) => const NavigationHomeScreen()), (Route<dynamic> route) => false);
  }

  /// Checking whether user active/inactive for using the app.
