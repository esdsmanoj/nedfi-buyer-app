...
import '../../model/user_model/check_user.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      getDetails(); // Now it's safe to call setState()
    });
    getToken();
  }
  ...
  Future getToken() async {
    if(Platform.isIOS){
      _token = await _firebaseMessaging.getAPNSToken();
    }else{
      _token = await _firebaseMessaging.getToken();
    }
  }

  /// Getting Language details from the API.
    ...
    return BaseWidget(
      ...
      child: SingleChildScrollView(
        child: Stack(children: [
          Image.asset("assets/images/login_background.png", height: MediaQuery.of(context).size.height, width: MediaQuery.of(context).size.width,fit: BoxFit.fill,),
          Positioned(
            left: -2,
            right: -2,
            top: 1,
            child: Padding(
              ...
              child:  Column(mainAxisAlignment: MainAxisAlignment.start, children: [
                ...
                Image.file(File(image), fit: BoxFit.contain, width: 153, height: 183, errorBuilder: (ctx, obj, st) {
                  return Image.asset(HeaderSingleton().splashImage, fit: BoxFit.contain, width: 153, height: 183);
                }),
                SizedBox(height: MediaQuery.of(context).size.height * 0.102),
                // Align(
              ...
              child: WidgetUtils.appTextWidget(
                ...
                Row(children: [
                  WidgetUtils.appTextWidget(
                      context: context, title: 'Country'.tr, fontSize: 16, family: 'Graphik', fontWeight: FontWeight.w500, textAlign: TextAlign.center, color: const Color(0xff111111)),
  ...
  /// Checking the login details from the SERVER API.
  Future login() async {
  ...
  /// Checking where user is farmer or not.
  void showISFarmer() {
  ...
  /// Getting the referral code details from the API.
  ...
  /// Checking the user registration details from SERVER.
  Future registration(String referral, {List<StepList>? steps}) async {
    ...
    isLoading.value = true;
    try {
      Map<String, dynamic> params1 = {"phone": mobileNo, "btn_submit": "submit", "device_id": _token};
