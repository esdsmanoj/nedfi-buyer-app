...
class SplashScreen extends StatefulWidget {
  final String route, splashImagePath, appDetailsName, domainName;
...
class _SplashScreenState extends State<SplashScreen> {
  bool isLoading = false, isImageLoaded = false;
  ...
  /// Showing snack bar to the bottom when error comes.
  ...
  /// Getting Domain details from the server.
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Center(child: Image.asset(widget.splashImagePath, alignment: Alignment.center, width: 200, height: 200)),
      ),
    );
  }

  /// Get starting details as per the domain configuration.
  ...
  /// Navigate between screens as per the client types.
  ...
  /// Checking if user active or not.
  ...
  Future fetchDomain() async {
    try {
      ...
      try {
        if(Platform.isIOS){
          await FirebaseMessaging.instance.getAPNSToken().then((value) => print("token:$value"));
        }else{
          await FirebaseMessaging.instance.getToken().then((value) => print("token:$value"));
        }
      ...
      headerParams = {"appname": name!, "X-API-KEY": 'CODEX@123', "domain": code!, "lang": lang, "client-type": "seller"};
