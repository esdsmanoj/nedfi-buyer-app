...
class UploadDocument extends StatefulWidget {
  final String buyerName, bidderId, productId, bidDate, isFrom,incentiveId;

  const UploadDocument({super.key, required this.buyerName, required this.bidderId, required this.productId, required this.bidDate, this.isFrom = "upload",required this.incentiveId});
  ...
  Future getBiddingDetails() async {
    try {
      ...
      if (viewReceiptModel.success == 1) {
        ...
        print(sellerInvoicePath + "/" + (viewReceipt?.data?[0].sellerInvoice ?? ""));
        imagePath=File((sellerInvoicePath + "/" + (viewReceipt?.data?[0].sellerInvoice ?? "")));
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      ...
      child: Scaffold( backgroundColor: Colors.white,
               ...
               true?Container(): Row(
                    ...
                    ? widget.bidDate != null
                        ? Row(
                            ...
                            children: [
                              ...
                              WidgetUtils.appTextWidget(
                                  context: context, title: getDateFormat(widget.bidDate ?? ""), color: const Color(0xff3F3F3F), fontWeight: FontWeight.w400, fontSize: 14, family: 'Graphik'),
                            ],
                          )
                        : Container()
                    : Container(),
                ...
                productData.value![0].statusTitle!.toLowerCase() == "sold" || productData.value![0].statusTitle!.toLowerCase() == "completed"
                  ...
                  children: [
                    ...
                    WidgetUtils.appTextWidget(
                        context: context, title:"₹" +((productData.value?[0].tradeProductBidding?.length)!>0 ?(productData.value?[0].tradeProductBidding?[0].bidPrice ?? ""):""), color: const Color(0xff3F3F3F), fontWeight: FontWeight.w400, fontSize: 14, family: 'Graphik'),
                  ],
                )
                    : Container(),
                const SizedBox(height: 12),
        ...
        bottomNavigationBar: /*widget.isFrom != "view"*/true
            ? widget.incentiveId!=""?Container(height: 1,):Padding(
        ...
        await request.send().then((response) async {
          response.stream.transform(utf8.decoder).listen((value) async {
            ...
            if (data['success'] == 1) {
              ...
              setState(() {
                productData.value![0].statusTitle = "Completed";
                statusColor = 0xff0074E8;
              });
