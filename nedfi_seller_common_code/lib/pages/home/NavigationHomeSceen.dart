...
import 'package:nedfi_seller_common_code/model/HomeCatagoryResponse.dart' as con;
...
class _NavigationHomeScreenState extends State<NavigationHomeScreen> {
  ...
  Future getDetails() async {
    try {
      ...
      setState(() {});
      HelperUtils().initLocation(context,(Tuple2<String, String> item) => HelperUtils().getWeather(item.item1, item.item2, () => setState(() {})), () => {});
      fetchSeason();
    } catch (e) {
      setState(() {});
    }
...
*/
  ...
  Future getCheckActive() async {
    try {
      final response = await APIService.getAPIMethod(url: ApiURL.checkActive+"/"+HeaderSingleton().paramsMaps!.userId!);
      final data = json.decode(response.body);
