...
class Steppers extends StatefulWidget {
  ...
  @override
  State<Steppers> createState() => _StepperState();
}

class _StepperState extends State<Steppers> with TickerProviderStateMixin {
  late List<GlobalKey> _keys;
  final Map<int, StepsState> _oldStates = <int, StepsState>{};
  ...
  bool _isLabel() {
    for (final Steps step in widget.steps) {
      if (step.label != null) {
        return true;
      }
    }
  ...
  Widget _buildCircleChild(int index, bool oldState) {
    final StepsState state = oldState ? _oldStates[index]! : widget.steps[index].state;
    final bool isDarkActive = _isDark() && widget.steps[index].isActive;
    assert(state != null);
    switch (state) {
      case StepsState.indexed:
      case StepsState.disabled:
        return Text(
          '${index + 1}',
          style: isDarkActive ? _kStepStyle.copyWith(color: Colors.black87) : _kStepStyle,
        );
      case StepsState.editing:
        return Icon(
          Icons.edit,
          color: isDarkActive ? _kCircleActiveDark : _kCircleActiveLight,
          size: 18.0,
        );
      case StepsState.complete:
        return Icon(
          Icons.check,
          color: isDarkActive ? _kCircleActiveDark : _kCircleActiveLight,
          size: 18.0,
  ...
  Widget _buildCircle(int index, bool oldState) {
    return Container(
      ...
      child: AnimatedContainer(
        ...
        child: Center(
          child: _buildCircleChild(index, oldState && widget.steps[index].state == StepsState.error),
        ),
      ),
    );
  }
  ...
  Widget _buildIcon(int index) {
    if (widget.steps[index].state != _oldStates[index]) {
      return AnimatedCrossFade(
        ...
        crossFadeState: widget.steps[index].state == StepsState.error ? CrossFadeState.showSecond : CrossFadeState.showFirst,
        duration: kThemeAnimationDuration,
      );
    } else {
      if (widget.steps[index].state != StepsState.error) {
      ...
      children: <Widget>[
        for (int i = 0; i < widget.steps.length; i += 1)
          Column(
            ...
            children: <Widget>[
              InkWell(
                onTap: widget.steps[i].state != StepsState.disabled ? () {
                ...
                canRequestFocus: widget.steps[i].state != StepsState.disabled,
