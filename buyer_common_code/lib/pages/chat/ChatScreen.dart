...
import '../../model/BidChat.dart';
import '../../model/helpdesk_login_model.dart';
import '../../model/trade_product_info.dart';

class ChatScreen extends StatefulWidget {
  String buyerId = "", farmerName = "", from = "";
  String? tradeProductBiddingId, prodId, isManage;

  ChatScreen(this.farmerName, this.from, {super.key, required this.buyerId, this.tradeProductBiddingId, this.prodId = "", this.isManage});
...
class _ChatterScreenState extends State<ChatScreen> {
            ...
            child: Scaffold(
                ...
                appBar: AppBar(
                  ...
                  title: WidgetUtils.appTextWidget(
                      context: context,
                      title: userChatDetails != null ? (userChatDetails?.sendToCilentDetails?.firstName ?? "") + " " + (userChatDetails?.sendToCilentDetails?.lastName ?? "") : widget.farmerName,
                            ...
                            productData.value?[0].statusTitle!.toLowerCase() != "live"
                              ...
                              Container(
                                child: ValueListenableBuilder(
                                  ...
                                  builder: (context, List<BidChatData>? value, child) {
                                    return value != null
                                        ? ListView.builder(
                        ...
                        ? (productData.value?.isNotEmpty ?? false)
                            ? (productData.value?[0].statusTitle?.toLowerCase()) == "completed"
                                ...
                                : Padding(
                                    ...
                                    child: Card(
                                      elevation: 1,
                                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8), side: const BorderSide(color: Color(0x33B4B6C4))),
                                      child: Container(
                                        height: 70,
                                        child: Row(
    ...
    try {
      ...
      if (connectionServerMsg != "key_check_internet") {
        ...
        if (chat.success == 1) {
          ...
          if (!isStatus) {
            getManageProductInformation(context, chat.tradeProductDetails!.tradeProductId!, () {
              isLoading.value = false;
              changeStatusColor();
              setState(() {});
            });
  ...
  addBidChat() async {
    try {
      ...
      if (connectionServerMsg != "key_check_internet") {
        Map<String, dynamic> params1 = {
          'send_from_id': HeaderSingleton().paramsMaps!.userId, //134
          'send_to_id': widget.buyerId, //3
